...
allowed-tools:
  ...
  - mcp__clockify-mcp__clockify__update_time_entry
  - mcp__clockify-mcp__clockify__delete_time_entry
  - mcp__clockify-mcp__clockify__report_summary
---

# Log this Claude Code session to Clockify

File a Clockify time entry for the work that just happened in this conversation. Be proactive but not annoying.

Optional duration hint: $ARGUMENTS (e.g. `45m`, `1h30m`; otherwise estimate from the conversation or ask).
...
2. **Pick the project.**
   ...
   - **Mapping heads-up**: if the project you chose isn't in the user's CLAUDE.md cheat-sheet (or there's no cheat-sheet at all), add a single one-liner at the *end* of your final response noting that the project wasn't in their cheat-sheet, so they can add a rule for it when convenient. Do not interrupt the flow, and do not edit the user's CLAUDE.md.

3. **Write a 1-sentence description.** Concrete, timesheet-readable a month from now. Good: "Refactored the auth middleware and added two regression tests". Bad: "worked on stuff" / "coded".
...
6. **Confirm with a one-liner**:
   *"About to log: 1h30m on <project> (today 14:00 to 15:30), '<description>'. OK?"*
